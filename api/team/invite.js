...
function inviteEmailHtml({ inviterName, workspaceName, link, role }) {
  // SECURITY (audit, May 2026): full five-char HTML escape so user-controlled
  // name / workspace fields can't break out of attribute contexts via `"`.
  const escapeHtml = (s) => String(s == null ? '' : s).replace(/[&<>"']/g, m => ({
    '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;'
  }[m]));
  const safeWs   = escapeHtml(workspaceName || 'a workspace');
  const safeName = escapeHtml(inviterName   || 'Someone');
...
<html><body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background:#F5F1E8; padding:32px; color:#0A0A0B;">
  <div style="max-width:520px; margin:0 auto; background:#FFFFFF; border-radius:16px; padding:32px;">
    ...
    <p style="margin:24px 0;">
      <a href="${safeLink}" style="display:inline-block; background:#0A0A0B; color:#F5F1E8; padding:12px 20px; border-radius:12px; text-decoration:none; font-weight:500; font-size:14px;">Accept invitation →</a>
    </p>
    <p style="font-size:12px; color:#8E8B84;">Or copy this link: <br/><span style="font-family: 'Geist Mono', monospace; color:#6B5BFF;">${safeLink}</span></p>
